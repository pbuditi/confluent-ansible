...
cd $BASE_DIR
ansible -vvvv -i $BASE_DIR/inventory/$DEPLOY_ENV/hosts.yml -u rioadmin -m ping preflight --private-key=$KEY_PATH

echo "ansible-playbook -i $BASE_DIR/inventory/$DEPLOY_ENV/hosts.yml $EXTRA_PARAMS $BASE_DIR/all.yml --private-key=$KEY_PATH"
ansible-playbook -vvvv -i $BASE_DIR/inventory/$DEPLOY_ENV/hosts.yml $EXTRA_PARAMS $BASE_DIR/all.yml --private-key=$KEY_PATH
