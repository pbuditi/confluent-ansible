...
# do
	echo "------------------------------- $(hostname) -------------------------------"
	# create pkcs store
	openssl pkcs12 -export -in $(hostname).cer -inkey  $(hostname).key  -name $(hostname) -password pass:confluent -out $(hostname -s).p12

	# convert pkcs to jks
	keytool -importkeystore -srckeystore $(hostname -s).p12 -srcstoretype PKCS12 -destkeystore $(hostname -s).keystore.jks -deststoretype jks --srcstorepass  confluent -deststorepass confluent -noprompt
	...
	keytool -noprompt -keystore $(hostname -s).keystore.jks -alias DBSBank-Root-CA-$(hostname -s) -import -file DBSBank-Root-CA.crt -storepass confluent -keypass confluent

        # Sign and import the host certificate into the keystore
#	keytool -noprompt -keystore $(hostname -s).keystore.jks -alias $(hostname) -import -file $(hostname).cer -storepass confluent -keypass confluent
